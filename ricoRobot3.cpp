        ...
        bool isDisplay)
{
    while( canMove( theBoard, robot, directionOffset, moveDirection)) {
        ...
        if(isDisplay)   //// project 6: board display depending on user option
            theBoard.display();
    }//end while( canMove...
...
int main()
{
    ...
    if (toupper(display) == 'T'){
        isDisplay = 1;
        displayIdentifyingInformationAndInstructions();  //// project 6, comment out to match output format
    }else{
        isDisplay = 0;
    }
    ...
    while( true) {
        ...
        moveRobotInIndicatedDirection( theBoard, robot, directionOffset, moveDirection,isDisplay);
